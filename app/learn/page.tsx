...
import { LearnResourceCard } from "@/components/content-card"
...
export default function LearnPage() {
  ...
  }, [searchQuery, selectedTypes, selectedLevels, selectedTrack])
  ...
  const resourcesByTrack = useMemo(() => {
    const grouped = allLearnResources.reduce(
      (acc, resource) => {
      ...
      {} as Record<string, typeof allLearnResources>,
    )
    return grouped
  }, [])
        ...
        {filteredResources.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredResources.map((resource) => (
              <LearnResourceCard key={resource.slug} resource={resource} />
            ))}
          </div>
        ) : (
          <Card>
            <CardContent className="p-12 text-center">
              ...
              <h3 className="text-xl font-semibold mb-2">没有找到相关资源</h3>
